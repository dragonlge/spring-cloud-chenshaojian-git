...
(function ($) {
    ...
    $.extend($.PaginationRenderers.defaultRenderer.prototype, {
        ...
        getLinks: function (current_page, eventHandler) {
            ...
            if (this.opts.show_first_last) {
                fragment.append(this.createLink(0, current_page, {text: this.opts.first_text, classes: ""}));
            }
            ...
            //last page
            if (this.opts.show_first_last) {
                fragment.append(this.createLink(np, current_page, {text: this.opts.last_text, classes: ""}));
            }

            if (this.opts.hasSelect) {
                var Jqselect = $("<select id='pageSelect' ><select>");
                for (var i = 1; i <= np; i++) {
                    if (i == current_page + 1) {
                        Jqselect.append("<option selected='selected' value='" + i + "'>" + i + " / " + np + "</option>");
                    } else
                        Jqselect.append("<option value='" + i + "'>" + i + " / " + np + "</option>");
                }
                fragment.append(Jqselect);
            }
            //show_total_info
            if (this.opts.show_total_info) {
                jQuery("<span style='border: 0px'>共有&nbsp;&nbsp;<span style=\"color:#3970AE;border: 0px;\" >" + this.maxentries + "</span>&nbsp;&nbsp;条记录&nbsp;&nbsp;&nbsp;</span>").appendTo(fragment);
            }
    ...
    $.fn.pagination = function (maxentries, opts) {
        ...
        function selectPage(new_current_page) {
            ...
            var continuePropagation = opts.callback(new_current_page, containers, opts.items_per_page);
            return continuePropagation;
        }
        ...
        if (opts.load_first_page) {
            opts.callback(current_page, containers, opts.items_per_page);
        }

...
var center_dialog = function (o) {
    ...
    //show动画 start
    c.css({
        width: 'auto', /*webkitTransform:'scale(.5) translate3d(0,-100px,0)',*/
    ...
    c.css({
        top: '50%',
        left: '50%',
        margin: -c.outerHeight() / 2 + 'px 0 0 -' + c.outerWidth() / 2 + 'px',
        visibility: 'visible',
        webkitTransition: '-webkit-transform .15s,opacity .15s'
    });
...
var dialog_main = function (options) {
    ...
    var $dialog = $(".dialog-tips").length > 0 ? $(".dialog-tips") : $(tpl);
    if (!options) {
        console.error("can not init dialog ,no options");
        return;
    }
    if (options.edge) {
        $("html").css("fontSize", 100);

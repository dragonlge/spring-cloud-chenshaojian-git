...
*          {
*              select:'.subbmit',
*              // @param {object} m：方法集合：m.hide()，m.rest();
 *              // @param {object} d：弹窗html元素
 *              handle:function(m,d){
*                  $.ajax({
*                      data: $(d).find('input').val(),
*                      success: m.hide
*                  });
*              }
 *          }
 *      ]
 *  });
 *  底部弹窗，适用于属性选择、地址选择等需求，具体调用形式如上或参考HTML页面
 */


var foot_dialog = function(o){
    var dialog = $('<div class="foot_dialog"></div>'),
        bg = $('<div class="bg"></div>'),
        c = $('<div class="c"></div>'),
        H = $(window).height(),
        w_sct = $(window).scrollTop(),
        timeStamp = Date.now(),
        isFirst = isNaN(parseInt($('html').data('dialog-timeStamp')));//是否为第一个弹窗

    //dialog可以多次叠加，如果是第一次
    //为html,body添加属性，禁止滚屏
    if(isFirst){
        $('html').data('dialog-timeStamp',timeStamp);
        $('html').addClass('dialog-pattern');
        $('body').css({height: w_sct + H});
    }

    //组合dialog
    dialog.append(bg).append(c).show();
    dialog.show();
    ...
    if(o.target){//传递selector
        ...
        if(o.clone === true){//o.clone默认undefined
            ...
            target.each(function(i,e){//将target取出并在原位置做标记待还原
                var flag = 'dialog_flag_'+Math.random().toString().slice(2,22);
                $(e).data('dialog_flag',flag).after('<mark id="'+flag+'"></mark>');
                c.append($(e).show());
            });
        }
    }else if(o.html){//拼接HTML
        c.html(o.html);
    }
    ...
    //show动画 start
    c.css({marginTop:H,visibility:'visible'});
    var tar = H  - c.outerHeight();
    tar = tar < 0 ? 0 : tar;
    c.animate({marginTop: tar}, 300, function () {
        //dialog可以多次叠加，如果是第一次
        //为html,body添加属性，禁止滚屏
        if(isFirst) {
            $('body').css({marginTop: -w_sct});
            $(window).scrollTop(0);
        }
        if (typeof o.callback === 'function') {
            o.callback(outer, c[0]);
        }
    });
    ...
    //outer将作为dialog返回对象和handle函数参数调用
    var outer = {
        //关闭弹层
        hide : function(callback){
            if(hide_flag === true){//短时多次触发关闭事件无效
                return;
            }
            hide_flag = true;
            if(o.bind && o.bind.length>=1){
                o.bind.forEach(function(obj){
                    c.find(obj.select).off('.dialog_handle');
                });
            }
            c.animate({marginTop:H},300);
            bg.animate({opacity:0},300,function(){
                if(o.target && o.copy !== true){
                    target.each(function(i,e){
                        var flag = $(e).data('dialog_flag');
                        $('#'+flag).before(e);
                        $(e).hide();
                        $('#'+flag).remove();
                        $(e).removeData('dialog_flag');
                    });
                }
                dialog.remove();

                if(isFirst){
                    //最后一个弹窗关闭时还原html,body状态
                    $('html').removeData('dialog-timeStamp');
                    $('html').removeClass('dialog-pattern');
                    $('body').css({height:'auto',marginTop:'auto'});

                    $(window).scrollTop(w_sct);


                    (function(){//确保安卓机型scroll成功
                        var ar = arguments;
                        if($(window).scrollTop() === 0 && w_sct !== 0){
                            setTimeout(function(){
                                $(window).scrollTop(w_sct);
                                ar.callee();
                            },1);
                ...
                if(typeof callback === 'function'){//可设置关闭后回调
                    callback();
                }
            });
        },
    ...
    if(o.bind && o.bind.length>=1){
        o.bind.forEach(function(obj){
            ...
            c.find(obj.select).on(type+'.dialog_handle',function(event){
                if(typeof obj.handle === 'function'){
                    ret = obj.handle.call(this,outer,c[0],event);
                }

            });
            //绑定关闭
            if(obj.close === true){
                c.find(obj.select).on('click.dialog_handle',function(){
                    //这里提供了一个方法，当handle类型为click(默认)，
                    //并且函数返回false时，阻止默认关闭事件
                    if(type !== 'click' || ret !==false){
                        outer.hide();
                    }
                });
...
/**
 * 居中弹窗
 * 使用方法与footer_dialog相同
 */

var center_dialog = function(o){
    var dialog = $('<div class="center_dialog"></div>'),
        bg = $('<div class="bg"></div>'),
        c = $('<div class="c"></div>');
    //组合dialog
    dialog.append(bg).append(c).show();
    ...
    //show动画 start
    c.css({width:'auto',/*webkitTransform:'scale(.5) translate3d(0,-100px,0)',*/webkitTransform:'translate3d(0,-50px,0)',opacity:0});
    c.css({top:'50%',left:'50%',margin: -c.outerHeight()/2+'px 0 0 -'+ c.outerWidth()/2+'px',visibility:'visible',webkitTransition:'-webkit-transform .15s,opacity .15s'});

    setTimeout(function(){
        c.css({webkitTransform:'translate3d(0,0,0)',opacity:1});
    },50);

    bg.animate({opacity:1},200,function(){
        if (typeof o.callback === 'function') {
            o.callback(outer, c[0]);
        }
    });
    ...
    var outer = {
        ...
        hide : function(callback){
            ...
            c.css({webkitTransform:'translate3d(0,-50px,0)',opacity:0});
        ...
        //当弹层高度改变时，重设弹窗位置
        reset : function(){
            c.css({margin: -c.outerHeight()/2+'px 0 0 -'+ c.outerWidth()/2+'px'});
        }
    };
 ...
 */
var alert = function(str,option){
    var str = str.toString(),
        dialog = $('<div class="alert"></div>'),
        word = $('<p>'+str+'</p>'),
        option = $.extend({timeout:1000},option),
        dialog_bg,
        btn;
 ...
 * @param {object} option 设定按钮，回调函数，当设定按钮时提示不再自动关闭
 */
var alertEmall = function(str,option){
    var str = str.toString(),
        dialog = $('<div class="alert"></div>'),
        word = $('<p>'+str+'</p>'),
        option = $.extend({timeout:1000},option),
...
var pull_loading = {
    ...
    init: function(o) {
        ...
        scrollElm.scroll(function() {
            ...
            if (condition) {
                ...
                var t = '加载中',
                    n = 1;
                if(!o.selector){
                    _this.loading.appendTo($('#wrap').length === 0?$('body'):$('#wrap'));
                }else{
                    if(!o.method){
                        $(o.selector).append(_this.loading);
                    }else{
                        switch(o.method){
                            case 'after':
                            ...
                            case 'before':
                                ...
                                break;
                            case 'prepend':
                                $(o.selector).prepend(_this.loading);
                                break;
                            case 'append':
                                $(o.selector).append(_this.loading);
                                break;
        ...
        clearTimeout(pull_loading.timeout);
        if(pull_loading.loading !== undefined){
            pull_loading.loading.remove();
        }
        pull_loading.flag = false;
    }
};
...
var push_loading = {
    ...
    init: function(o) {
        ...
        scrollElm.scroll(function() {
            ...
            var condition = scrollElm.scrollTop() <= 0 ;
        ...
        clearTimeout(push_loading.timeout);
        if(push_loading.loading !== undefined){
            push_loading.loading.remove();
        }
        push_loading.flag = false;
    }
};
 ...
 *
 * 使用touchend模拟click*
 * 当click在手机上第一次点击无法触发事件时使用*
 *
 * */
$.fn.touchend_click = function(callback){
    ...
    $(this).each(function(i,e){
        ...
        if(!isMobileClient){
            $(this).on('click.touchend_click',function(event){
                callback.call(e,event);
            });
            return;
        }

        $(this).data('touchend_click',callback);
        ...
        e.addEventListener('touchend',function(event){
            if(move === false){
                if(typeof $(this).data('touchend_click') === 'function'){
                    $(this).data('touchend_click').call(e,event);
                }
            }
            move = false;
        },false);
...
$.fn.trigger_touchend_click = function(){//手动触发
    $(this).each(function(i,e){
        if(!isMobileClient){
            $(this).trigger('click.touchend_click');
            return;
        }
        if(typeof $(this).data('touchend_click') === 'function') {
            $(this).data('touchend_click').call(e);
        }
    });
    return this;
};

$.fn.unbind_touchend_click = function(){//解除绑定
    $(this).each(function(i,e){
        if(!isMobileClient){
            $(this).off('click.touchend_click');
            return;
        }
        $(this).removeData('touchend_click');
    });
    return this;
};
/***
 * edge为true是兼容模式，兼容以前老的弹出框
 *  tips_dialog({title:"12",content:"222",btn:'12',edge:false,link:function(e){e.stopPropagation();console.log(1)}})
 * @param options op
 */
var tips_dialog = function(options){
    dialog_main({
        title:options.title,
        ...
        buttons:[{
            text:options.btn||"",
            callback:options.link|| $.noop
        }],
        old:true
    });
    /**
     var tpl = '<div class="dialog-tips">'+
     ...
     '</div>'+
     '<hr/>'+
     '<a class="tips-btn" href="javascript:;">--</a>'+
     '</div>'+
     '</div>';
     var $dialog = $(".dialog-tips").length>0?$(".dialog-tips"):$(tpl);
     if(!options){console.error("can not init dialog ,no options");return;}
     if(options.edge){
     ...
     $dialog.find(".tips-btn").text(options.btn||"").off("click").on("click",function(e){
        var ck = options.link|| $.noop;
        ck(e);
        $dialog.hide()
    });
     $dialog.appendTo("body");
     $dialog.find(".dialog-wrap").css({
        top:$(window).height()/2-($dialog.find(".dialog-wrap").height()/2)
    }).show();
     if($.isNumeric(options.time)){
 ...
 * api例子:dialog_main({title: "12", content: "", edge: true,
        buttons:[{
            ...
            text:"123",
            callback:function(){console.log(1231)}
        }]
    })
 * @param options
 */
var dialog_main = function(options){
    var tpl = '<div class="dialog-tips">'+
        '<div class="dialog-wrap">'+
        '<div class="dialog-body">'+
        '<h2 class="tips-title">--</h2>'+
    ...
    if(options.buttons){
        for(var btnIndex in options.buttons){
            var $btn = $('<a class="tips-btn" href="javascript:;">--</a>').text(options.buttons[btnIndex].text);
            $btn.on("click",function(e){
                var ck = options.buttons[btnIndex].callback|| $.noop;
                ck(e);
                closeDialog();
            });
            !options.old && $btn.css({

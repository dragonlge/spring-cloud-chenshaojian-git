...
var pageaction = function(){
    $.get('./list?t='+new Date().getTime(),{
        name:$("#name").val(),created:$("#created").val()
    },function(data){
...
var pageselectCallback = function(page_index, jq, size){
    if(currentPageData!=null){
        ...
        $.get('./list?t='+new Date().getTime(),{
            size:size,page:page_index,name:$("#name").val(),created:$("#created").val()
        },function(data){
...
function fillData(data){
    ...
    $.each(data,function(k,v){
        ...
        html += '<tr> ' +
            ...
            '<td>'+ (v.created==null?'': getSmpFormatDateByLong(v.created,true)) +'</td>' ;
        html += '<td><a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="edit(\''+ v.id+'\')">编辑</a><a class="c-50a73f mlr-6" href="javascript:void(0)" onclick="del(\''+ v.id+'\')">删除</a></td>' ;
        html +='</tr>' ;

                    ...
                    if (h == 116) {
                        ...
                        if ($(e).data("set") !== true)
                            $(e).css({
                                visibility: "visible",
                                opacity: 0
                            }).data("set", true).stop().animate({opacity: 1}, 300);
                        return;
                    }
                    if (h < 116 && parseInt($(e).css("marginTop")) != parseInt((116 - h) / 2)) {
                        $(e).css("marginTop", parseInt((116 - h) / 2)).css({
                            visibility: "visible",
                            opacity: 0
                        }).data("set", true).stop().animate({opacity: 1}, 300);
                    }
...
function sureChoose(callback) {
    ...
    if (isUpPic) {
        var fileUrl = $("#view > img").attr("src"), left_up = $("#p1").val(), left_bottom = $("#p4").val(),
            right_up = $("#p2").val(), right_bottom = $("#p3").val();
